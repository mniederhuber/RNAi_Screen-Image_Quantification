...
loose_ends <- nr %>% 
  dplyr::select(id, date, scene, size) %>%
  dplyr::filter(size == '1024x1024') %>%
  dplyr::group_by(id) %>%
  dplyr::count() %>%
  dplyr::arrange(n) %>%
  dplyr::filter(n <= 4) 

write.csv(loose_ends, 'r_out/loose_ends.csv', quote = F, sep = ',', row.names = F, col.names = T)
...
outdata %>%
  dplyr::filter(stack >= 10 & gfp >= 5 & size == "1024x1024") %>%
  dplyr::group_by(symbol, line) %>%
  dplyr::mutate(med = median(KDtoWT)) %>%
